...
def test_promote_local_moves_the_rule_preserving_its_relative_path(
    ...
    repo = make_repo(home)
    local = write_rule(
        repo / ".byolsp" / "rules" / "personal" / "local" / "python" / "exp.yml", "exp"
    )
    ...
    destination = repo / ".byolsp" / "rules" / "project" / "python" / "exp.yml"
    assert destination.read_text() == content
    assert not local.exists()
    out = capsys.readouterr().out
    assert "Promoted 'exp' to .byolsp/rules/project/python/exp.yml" in out
...
def test_promote_local_with_keep_local_fails_before_writing(
    ...
    repo = make_repo(home)
    local = write_rule(
        repo / ".byolsp" / "rules" / "personal" / "local" / "exp.yml", "exp"
    )

    assert promote(repo, "exp", "local", "--keep-local") == 1

    assert local.is_file()
    assert not (repo / ".byolsp" / "rules" / "project" / "exp.yml").exists()
...
def test_promote_global_copies_without_touching_canonical_or_exclusions(
    ...
    destination = repo / ".byolsp" / "rules" / "project" / "no-cast.yml"
    assert destination.read_text() == canonical.read_text()
    assert canonical.is_file()
    assert not (mirror(repo) / "no-cast.yml").exists()
    assert "excluded_rule_ids: []" in (repo / ".byolsp" / "local.yml").read_text()
...
def test_promote_requires_replace_to_overwrite_the_destination(
    ...
    existing = write_rule(
        repo / ".byolsp" / "rules" / "project" / "exp.yml", "old-exp", message="Old."
    )
    local = write_rule(
        repo / ".byolsp" / "rules" / "personal" / "local" / "exp.yml",
        "exp",
        message="New.",
    )
