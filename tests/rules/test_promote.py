...
def test_promote_local_moves_the_rule_preserving_its_relative_path(
    ...
    repo = make_repo(home)
    local = write_rule(repo / ".byor" / "rules" / "personal" / "local" / "python" / "exp.yml", "exp")
...
def test_promote_local_with_keep_local_fails_before_writing(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
    repo = make_repo(home)
    local = write_rule(repo / ".byor" / "rules" / "personal" / "local" / "exp.yml", "exp")
...
def test_promote_requires_replace_to_overwrite_the_destination(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
    repo = make_repo(home)
    ...
    existing = write_rule(repo / ".byor" / "rules" / "project" / "exp.yml", "old-exp", message="Old.")
    local = write_rule(
...
def test_promote_unknown_rule_id_fails_cleanly(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
    repo = make_repo(home)
...
def test_promote_package_rule_copies_it_into_project_and_keeps_the_source(
    ...
    source = write_package_rule(home, "python-strict", relpath="python/no-cast.yml", rule_id="pkg-cast")
    repo = make_repo(home)
    ...
    assert not (repo / ".byor" / "rules" / "personal" / "packages" / "python-strict").exists()
    assert "Promoted 'pkg-cast'" in capsys.readouterr().out


def test_promote_check_vendors_a_package_check_into_repo_config(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_promote_check_vendors_a_global_check_into_repo_config(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_promote_check_that_is_already_a_repo_check_fails(home: Path, capsys: pytest.CaptureFixture[str]) -> None:
    write_package_check(home, "python-strict", name="pkg-ruff", run="ruff-check")
    repo = make_repo(home)
    install_package(repo, "python-strict")
    assert main(["promote", "--repo", str(repo), "--check", "pkg-ruff"]) == 0
