...
# `pre-commit install` renames byor's gate shim to pre-commit.legacy: the gate
# still chains through .legacy, but byor no longer manages it. The marker on
# the .legacy file is the evidence that makes this state verifiable.
def test_doctor_flags_a_gate_shim_displaced_by_pre_commit_install(
    home: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    repo = home / "repo"
    repo.mkdir()
    ...
    out = capsys.readouterr().out
    assert "FAIL  git_shims" in out
    assert ".git/hooks/pre-commit was displaced to pre-commit.legacy by `pre-commit install`" in out
    assert "run `byor init --private --gate` to reclaim or keep as-is" in out
