...
def install(*extra: str) -> int:
    return main(["install", "--non-interactive", *extra])


def test_install_sets_up_the_global_ast_grep_config_and_skill(home: Path) -> None:
    assert install("--agents", "claude-code") == 0
...
def test_unknown_agent_fails_cleanly(
    ...
    assert install("--agents", "frobnicate") == 1
